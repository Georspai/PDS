...
void calculate_grid(int* numOfBlocks, int* numOfThreads, int lattice_size);

__host__ void Ising_comp(int8_t** G, int8_t** G_trans, int *size, int *steps,int numofBlocks, int numofThreads);

__host__ void swap_dptr(void** x, void** y);

__host__ __device__ int8_t sgn(int8_t x);

__global__ void compute_State(int8_t* a, int8_t* b, int *size);


int main(int argc, char* argv[]) {
    
    //n:Lattice Dimension & k:Number of Iterations
    int n=7000, k=10, cnvrg=0;
    int* dev_n;
    int* dev_cnvrg;
    ...
    printf("GPU ISING COMPUTATION V1\n\n");
    ...
    calculate_grid(&numOfBlocks, &numOfThreads, n);
    cudaEventRecord(start);

    Ising_comp(&dev_lattice, &dev_lattice_trans, dev_n, &k,numOfBlocks, numOfThreads);
  ...
  /*  for (int i = 0; i < n; i++)
    ...
    cudaEventElapsedTime(&milliseconds, start, stop);
    printf("\nIterations(k)\tDim(n)\tMaxThreadNumPerBlock\tNumofBlocks\tnumOfThreads\tElapsed time (ms)\n");
    printf("%d\t%d\t1024\t%d\t%d\t%f\n",k,n,numOfBlocks,numOfThreads, milliseconds);
...
__host__ void Ising_comp(int8_t** lat, int8_t** lat_trans, int *size, int *steps, int numofBlocks ,int numofThreads)
{
    int8_t* G = *lat;
    ...
    {   
        //cudaDeviceSynchronize();
        compute_State <<<numofBlocks, numofThreads >>> (*lat, *lat_trans, size);
 ...
 __global__ void compute_State(int8_t* G, int8_t* G_trans, int* size)
{    
     int n = *size;
...
void calculate_grid(int* numOfBlocks, int* numOfThreads, int lattice_size) {
    ...
    block_num = lat_points / maxThreadsPerBlock + ((lat_points % maxThreadsPerBlock) > 0);
    thread_num = lat_points / block_num + ((lat_points % block_num) > 0);
